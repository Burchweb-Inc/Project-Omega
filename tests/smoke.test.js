...
assert.match(index, /\['about', 'support', 'privacy', 'terms', 'understand', 'beta'\]/, 'Public information pages should render through the shared shell.');
...
assert.match(app, /app\.get\('\/beta'/, 'The beta page should have a public route.');
assert.match(fs.readFileSync(path.join(__dirname, '..', 'views', 'partials', 'dashboard.ejs'), 'utf8'), /data-dashboard-tab|Overview|Groups|Moderate/, 'The dashboard should offer tabbed sections.');
assert.match(fs.readFileSync(path.join(__dirname, '..', 'views', 'partials', 'site-admin.ejs'), 'utf8'), /data-group-search|Visit group/, 'The site admin dashboard should include a searchable group list and visit links.');
assert.ok(pkg.dependencies.marked, 'Announcements should have a Markdown parser dependency.');
