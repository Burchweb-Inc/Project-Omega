...
function loadOrgsFromDatabase() {
  ...
  orgs.splice(0, orgs.length, ...rows.map((row) => {
    ...
    org.courses = (org.courses || []).map((course) => ({ ...course, items: (course.items || []).map((item) => ({ ...item, comments: item.comments || [], verifiedBy: item.verifiedBy || [], downvotedBy: item.downvotedBy || [] })), board: course.board || [], resources: course.resources || [] }));
...
function persistState() {
  ...
  users.forEach((user) => userWrite.run({
    ...
    tasks: JSON.stringify(user.tasks || [])
  }));
...
function render(request, response, page, extra = {}) { response.render('index', { page, view: page, user: request.user, users, orgs, canEdit, selectedOrg: null, error: request.query?.error, ...extra }); }
...
io.on('connection', (socket) => {
  socket.on('course:join', ({ orgId, courseId } = {}) => {
    ...
    if (canAccess(org, socket.user) && course) socket.join(courseRoom(org.id, course.id));
  });
...
app.get('/org/:id/course/:courseId', requireUser, (request, response) => {
  ...
  const typeOrder = { Homework: 1, 'Upcoming test': 2, Project: 3, Event: 4 };
  renderOrgPage(request, response, 'course', { course: { ...course, items: [...course.items].sort((left, right) => (typeOrder[left.type] || 9) - (typeOrder[right.type] || 9)) } });
});
...
app.post('/org/:id/courses/:courseId/items', requireUser, (request, response) => {
  ...
  const item = { id: id(), title: String(request.body.title || '').trim(), type: request.body.type, due: request.body.due || 'No date', done: request.body.type !== 'Event' ? false : null, comments: [], verifiedBy: [], downvotedBy: [], createdBy: request.user.id };
...
app.post('/org/:id/items/:itemId/downvote', requireUser, (request, response) => {
  ...
  persistState(); const course = org.courses.find((entry) => entry.items.includes(item)); emitCourse(org, course, 'item:downvote-changed', { itemId: item.id, downvotedBy: item.downvotedBy });
  return sendMutation(request, response, { itemId: item.id, downvotedBy: item.downvotedBy }, `/org/${org.id}/course/${course.id}`);
});
...
app.post('/org/:id/items/:itemId/update', requireUser, (request, response) => {
  ...
  const title = String(request.body.title || '').trim(); if (!title) return sendMutation(request, response, { error: 'A title is required.' }, `/org/${org.id}`);
  item.title = title; item.type = request.body.type || item.type; item.due = request.body.due || 'No date'; persistState(); const course = org.courses.find((entry) => entry.items.includes(item)); emitCourse(org, course, 'item:updated', { item });
  return sendMutation(request, response, { item }, `/org/${org.id}/course/${course.id}`);
});
...
app.post('/org/:id/items/:itemId/delete', requireUser, (request, response) => {
  ...
  course.items = course.items.filter((entry) => entry.id !== item.id); org.groups = (org.groups || []).filter((group) => group.itemId !== item.id); persistState(); emitCourse(org, course, 'item:deleted', { itemId: item.id });
  return sendMutation(request, response, { itemId: item.id }, `/org/${org.id}/course/${course.id}`);
});
...
app.post('/org/:id/items/:itemId/todo', requireUser, (request, response) => {
  ...
  let task;
  if (org && course && item) { request.user.tasks ||= []; task = request.user.tasks.find((entry) => entry.sourceId === item.id); if (!task) { task = { id: id(), title: item.title, sourceId: item.id, source: `${course.name} · ${org.name}`, due: item.due, priority: 'Normal', done: false }; request.user.tasks.push(task); persistState(); emitCourse(org, course, 'task:added', { userId: request.user.id, task }); } }
  return sendMutation(request, response, { task: task || null }, `/org/${org?.id || ''}/course/${course?.id || ''}`);
});

app.post('/tasks/:taskId/toggle', requireUser, (request, response) => {
  const task = (request.user.tasks || []).find((entry) => entry.id === request.params.taskId); if (task) task.done = !task.done; persistState(); return sendMutation(request, response, { task: task || null }, '/todo');
});

app.post('/tasks', requireUser, (request, response) => {
  let task = null;
  if (request.body.title?.trim()) { request.user.tasks ||= []; task = { id: id(), title: request.body.title.trim(), sourceId: null, source: 'Personal study goal', due: request.body.due || 'No date', priority: request.body.priority || 'Normal', done: false }; request.user.tasks.push(task); persistState(); }
  return sendMutation(request, response, { task }, '/todo');
});
...
app.post('/org/:id/items/:itemId/comments', requireUser, (request, response) => {
  const org = getOrg(request); const item = canAccess(org, request.user) ? org.courses.flatMap((course) => course.items).find((entry) => entry.id === request.params.itemId) : null;
  let comment;
  if (item && String(request.body.comment || '').trim()) { comment = { id: id(), author: request.user.name, userId: request.user.id, text: String(request.body.comment).trim(), createdAt: new Date().toISOString() }; item.comments.push(comment); persistState(); const course = org.courses.find((entry) => entry.items.includes(item)); emitCourse(org, course, 'item:comment-added', { itemId: item.id, comment }); }
...
app.post('/org/:id/breakout/:groupId/tasks/:taskId/update', requireUser, (request, response) => {
  ...
  task.title = String(request.body.title || '').trim() || task.title; task.done = request.body.done === 'true' || request.body.done === 'on'; persistState(); emitGroup(org, 'breakout:task-updated', { groupId: group.id, task });
  return sendMutation(request, response, { task }, `/org/${org.id}/breakout/${group.id}`);
});
...
app.post('/org/:id/course/:courseId/board/:cardId/delete', requireUser, (request, response) => {
  ...
  return sendMutation(request, response, { cardId: request.params.cardId }, `/org/${org.id}/course/${course.id}#board`);
});
